...
def load_fasta(atg, *control):
	...
	for line in open('/net/home.isilon/ag-russell/bq_gsingh/gpcr/class_A_gpcr.fasta', 'r'):
		...
		if line[0] == '>':
			if (line.split(' ')[0].replace('>', '').split('|')[1].split('|')[0] in atg) == True:
				name = atg[line.split(' ')[0].replace('>', '').split('|')[1].split('|')[0]]
			else:
				continue

			if (name in fasta) == False:
				if len(control) != 0:
...
def hmm_search(obj, path, temp_path):
	l = ''
	for name in obj:
		l += '>' + str(name) + '\n'
		l += str(obj[name].seq) + '\n'
	open(temp_path+'/temp_fasta_file.txt', 'w').write(l)
	os.system('hmmsearch '+path+'/data_precog/7tm_1.hmm '+temp_path+'/temp_fasta_file.txt > '+temp_path+'/temp_hmm_file.txt')
