...
for dimr in ['PCA']:
    ...
    for assay in ['STRING', 'IUPHAR', 'ebBRET', 'Shedding']:
        ...
        for gprotein in gproteins:
            for layer in range(0,34):
                if dimr == 'PCA':
                    df = pd.read_csv('embedding_all/'+gprotein+'_All_'+str(layer)+'_'+dimr+'.tsv', sep='\t')
                #else:
                #    df = pd.read_csv('embedding_all/'+gprotein+'_All_'+str(layer)+'_'+dimr+'.tsv', sep='\t')
                ...
                print (dimr + '\t' + gprotein + '\t' + str(layer) + '\t' + assay + '\t' + str(round(score,3)))
    ...
    df = pd.DataFrame(data, columns=['DimR', 'Assay', 'G-protein', 'Layer', 'Score'])
    df = df.sort_values(by=['Assay', 'G-protein', 'Score'], ascending=[False, False, False])
    df.to_csv('systematic'+dimr+'Best.tsv', sep='\t', index=False)
...
for dimr in ['PCA']:
    ...
    df = pd.DataFrame(data, columns=['DimR', 'Layer', 'Score'])
    df = df.sort_values(by=['Score'], ascending=[False])
    df.to_csv('systematicClass'+dimr+'Best.tsv', sep='\t', index=False)
'''
...
for family in families:
    ...
    for dimr in ['PCA']:
        ...
        df = df.sort_values(by=['Score'], ascending=[False])
        df.to_csv('systematicFamilyBest/'+family.replace(' ', '_').replace('>', '_').replace('<', '_').replace('/', '_')+'_'+dimr+'Best.tsv', sep='\t', index=False)

        dfAllFamilies = pd.concat([dfAllFamilies, df])
        #sys.exit()

dfAllFamilies.to_csv('systematicFamilyBest/AllFamiliesBest.tsv', sep='\t', index=False)
